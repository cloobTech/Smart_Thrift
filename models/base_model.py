...
class BaseModel:
    ...
    id: Mapped[str] = mapped_column(String(60), nullable=False, primary_key=True)
    ...
    def __set_attrs(self, attr_dict: dict) -> None:
        ...
        if "created_at" not in attr_dict:
            ...
            attr_dict['.created_at'] = datetime.strptime(attr_dict["created_at"], time_format)
        if "updated_at" not in attr_dict:
            attr_dict['updated_at'] = datetime.now()
        elif "updated_at" in attr_dict and type(attr_dict["updated_at"]) is str:
            attr_dict['updated_at'] = datetime.strptime(attr_dict["updated_at"], time_format)
